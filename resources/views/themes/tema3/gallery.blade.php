@php
    // --- KODE INI WAJIB ADA UNTUK MENGAMBIL DATA GALERI ---
    // Logika untuk mengambil data gallery dari Model Gallery
    // Diasumsikan Model App\Models\Gallery sudah diimport atau menggunakan FQCN.
    // Jika Anda belum mengimpor App\Models\Gallery, pastikan baris ini diletakkan 
    // di file view Blade (di mana App\Models\Gallery sudah dapat diakses).
...
<div id="galeri" class="container mx-auto px-4">
            ...
            @foreach($galleries as $img)
                <div class="relative group" data-aos="zoom-in" data-aos-delay="{{ $loop->iteration * 100 }}">
                    ...
                    <a href="{{ asset('storage/'.$img->image) }}" 
                        ...
                        {{-- Shadow dan border Emas --}}
                        class="block w-full aspect-square overflow-hidden rounded-xl shadow-xl cursor-pointer bg-slate-200 relative border border-amber-50">
                        ...
                        <div class="absolute inset-0 bg-slate-900/50 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center backdrop-blur-sm">
    ...
    @else
        {{-- Empty State (Aksen Emas) --}}
        <div class="text-center py-12 bg-amber-50 rounded-3xl border border-dashed border-amber-300" data-aos="fade-up">
            <div class="inline-flex items-center justify-center w-16 h-16 rounded-full bg-amber-100 mb-4">
                <i class="bi bi-images text-3xl text-amber-500"></i>
            </div>
