<div id="mempelai" class="container mx-auto px-4 relative z-10">
    ...
    <div class="text-center max-w-3xl mx-auto mb-12" data-aos="fade-up">
        {{-- Aksen Emas Tua --}}
        <span class="text-amber-700 font-bold tracking-wider uppercase text-xs md:text-sm mb-2 block">
            Tasyakur Khitan
        </span>
        {{-- Font Classic --}}
        <h2 class="font-serif-classic text-4xl md:text-5xl text-slate-800 mb-6">
            Ananda Tercinta
        </h2>
    ...
    <div class="relative max-w-4xl mx-auto" data-aos="fade-up" data-aos-delay="200">
        ...
        {{-- Background Pattern (Hiasan Layer 1: Warna Emas Muda Transparan) --}}
        ...
        <div class="relative z-10 bg-white rounded-[2.5rem] shadow-2xl p-8 md:p-12 text-center border border-amber-50">
            ...
            <div class="relative inline-block mb-8">
                {{-- Lingkaran Dekorasi di belakang foto (Aksen Emas) --}}
                ...
                <div class="relative p-2 bg-white rounded-full shadow-lg">
                    @if($event->child_photo)
                        <img src="{{ asset('storage/'.$event->child_photo) }}" 
                            ...
                            {{-- Border Emas Muda --}}
                            class="w-48 h-48 md:w-64 md:h-64 rounded-full object-cover border-4 border-amber-200 shadow-inner"
                            loading="lazy">
                    @else
                        {{-- Placeholder jika tidak ada foto --}}
                        <div class="w-48 h-48 md:w-64 md:h-64 rounded-full bg-slate-100 flex items-center justify-center border-4 border-amber-200 text-slate-300">
                            <i class="bi bi-person-fill text-6xl"></i>
                        </div>
                    @endif
                </div>

                {{-- Ikon Hiasan kecil (Aksen Emas Tua) --}}
                <div class="absolute bottom-2 right-2 bg-amber-800 text-white w-10 h-10 rounded-full flex items-center justify-center shadow-lg border-2 border-white">
            ...
            {{-- Nama & Orang Tua --}}
            <div class="space-y-4">
                {{-- Nama Anak (Font Classic & Aksen Emas Tua) --}}
                <h3 class="font-serif-classic text-5xl md:text-6xl text-amber-800 drop-shadow-sm">
                    {{ $event->child_name }}
                </h3>

                <div class="flex items-center justify-center gap-3 text-slate-500 my-4">
                    {{-- Garis Pemisah Emas --}}
                    <span class="h-px w-12 bg-amber-200"></span>
                ...
                @if(!empty($event->turut_mengundang_ayah) || !empty($event->turut_mengundang_ibu))
                    ...
                    <div class="mt-10 pt-8 border-t border-dashed border-amber-200 w-full max-w-2xl mx-auto">
                        <p class="text-xs font-bold text-amber-700 uppercase tracking-[0.2em] mb-6">
                            Turut Mengundang
                        </p>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 text-center md:text-left">
                            ...
                            <div>
                                <h4 class="font-semibold text-amber-800 mb-3 text-base uppercase tracking-wide">
                                    Kel. Besar Bapak
                                </h4>
            ...
            {{-- Alamat (Aksen Hover Emas) --}}
            <div class="mt-10 inline-flex items-center justify-center gap-2 px-6 py-3 bg-slate-50 rounded-full border border-slate-200 text-sm md:text-base text-slate-600 hover:bg-amber-50 hover:border-amber-200 hover:text-amber-700 transition-colors cursor-default group">
                {{-- Icon Merah menjadi Emas Tua --}}
                <i class="bi bi-geo-alt-fill text-amber-600 group-hover:scale-110 transition-transform"></i>
