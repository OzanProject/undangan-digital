<div id="rsvp" class="container mx-auto px-4">
    ...
    <div class="grid md:grid-cols-12 gap-8 lg:gap-12 items-start">
        ...
        <div class="md:col-span-5 lg:col-span-5" data-aos="fade-right">
            {{-- Card Form: Shadow Lebih Dalam & Border Emas --}}
            <div class="bg-white rounded-3xl shadow-2xl border border-amber-50 p-6 md:p-8 sticky top-24">
                ...
                <form id="rsvp-form" method="POST" action="{{ route('wishes.store') }}#rsvp-form">
                    ...
                    <div class="mb-5">
                        ...
                        <input type="text" 
                                ...
                                {{-- Focus Style Emas --}}
                                class="w-full px-5 py-3 rounded-xl bg-slate-50 border-slate-200 focus:bg-white focus:border-amber-500 focus:ring-2 focus:ring-amber-100 transition-all outline-none text-slate-800 font-medium placeholder:text-slate-400 placeholder:font-light"
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="message" 
                                 rows="4" 
                                 {{-- Focus Style Emas --}}
                                 class="w-full px-5 py-3 rounded-xl bg-slate-50 border-slate-200 focus:bg-white focus:border-amber-500 focus:ring-2 focus:ring-amber-100 transition-all outline-none text-slate-800 font-medium placeholder:text-slate-400 placeholder:font-light resize-none"
                                 placeholder="Tuliskan doa terbaik untuk ananda..." 
                                 required>{{ old('message') }}</textarea>
                    </div>

                    {{-- Tombol Kirim (Aksen Emas Tua) --}}
                    <button type="submit" class="w-full py-3.5 rounded-xl bg-amber-800 text-white font-bold shadow-xl shadow-amber-900/40 hover:bg-amber-700 hover:-translate-y-0.5 transition-all duration-300 flex items-center justify-center gap-2">
        ...
        <div class="md:col-span-7 lg:col-span-7" data-aos="fade-left">
            {{-- Card List: Shadow Lebih Dalam & Border Emas --}}
            <div class="bg-white rounded-3xl shadow-2xl border border-amber-50 h-[600px] flex flex-col relative overflow-hidden">
                
                {{-- Header List (Aksen Emas) --}}
                <div class="p-5 border-b border-amber-100 bg-amber-50 flex justify-between items-center">
                    <h3 class="font-bold text-slate-700">Ucapan Terbaru</h3>
                    {{-- Badge Jumlah Pesan (Aksen Emas) --}}
                    <span class="px-3 py-1 rounded-full bg-white border border-amber-200 text-xs font-bold text-amber-700 shadow-sm">
                        {{ count($wishes) }} Pesan
                    </span>
                </div>
                ...
                <div class="flex-1 overflow-y-auto p-5 space-y-6 custom-scrollbar">
                    @forelse($wishes as $wish)
                        @php
                            ...
                            // Mengganti warna Avatar agar tetap elegan dan menggunakan tone Emas/Cokelat/Netral
                            $colors = ['bg-amber-100 text-amber-700', 'bg-stone-200 text-stone-700', 'bg-red-100 text-red-700', 'bg-slate-200 text-slate-700', 'bg-yellow-100 text-yellow-700'];
                        ...
                        <div class="animate-fade-in-up">
                            <div class="flex gap-4">
                                ...
                                <div class="flex-1 space-y-2">
                                    ...
                                    {{-- Balasan Admin (Aksen Emas) --}}
                                    @if($wish->reply)
                                        <div class="flex gap-3 justify-end animate-fade-in-up" style="animation-delay: 0.1s;">
                                            {{-- Bubble Balasan --}}
                                            <div class="bg-amber-50 rounded-2xl rounded-tr-none p-3 border border-amber-100 shadow-sm max-w-[90%] text-right">
                    ...
                    @empty
                        <div class="h-full flex flex-col items-center justify-center text-amber-400 opacity-60">
...
<script>
    ...
    Swal.fire({
        ...
        confirmButtonColor: '#F59E0B', // Amber 600 (Aksen Emas Tema 3)
        timer: 3000,
