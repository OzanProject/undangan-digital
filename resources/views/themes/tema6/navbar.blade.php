<style>
    /* ---------------------- Custom CSS untuk Tema 6 ---------------------- */
    
    /* Global Background untuk Ungu Lembut */
    body {
...
<script>
    Swal.fire({
        ...
        confirmButtonColor: '#5b21b6', // Violet 700 (Aksen Tema 6)
        timer: 3000,
...
>
    ...
    <nav 
        :class="scrolled 
            ? 'translate-y-0 bg-white/95 shadow-lg backdrop-blur-md border-b border-violet-100' 
            : '-translate-y-full'"
    ...
    >
        <div class="container mx-auto flex items-center justify-between px-4 md:px-8">
            ...
            {{-- Logo / Brand (Aksen Violet Tua) --}}
            <a href="#hero" class="flex items-center gap-2 group">
                <div class="w-9 h-9 rounded-full bg-violet-100 text-violet-800 flex items-center justify-center">
                    <i class="bi bi-balloon-heart-fill text-lg"></i>
                </div>
                <span class="font-serif-elegan text-2xl font-bold text-slate-800 group-hover:text-violet-800 transition">
            ...
            <div class="hidden lg:flex items-center space-x-1">
                <template x-for="item in menu" :key="item.link">
                    <a 
                        ...
                        {{-- Menu Aktif: bg-violet-700 --}}
                        :class="(active == item.link.replace('#', '')) 
                            ? 'bg-violet-700 text-white shadow-md shadow-violet-300' 
                            {{-- Menu Hover: hover:bg-violet-100, hover:text-violet-800 --}}
            ...
            <button 
                @click="open = true"
                class="lg:hidden p-2 text-slate-700 hover:bg-violet-100 hover:text-violet-800 rounded-lg transition"
            >
    ...
    <div 
        ...
        class="fixed right-0 top-0 h-full w-72 bg-white shadow-2xl z-[100] lg:hidden flex flex-col border-l border-violet-100"
    >
        {{-- Drawer Header (Aksen Violet Tua) --}}
        <div class="flex items-center justify-between p-5 bg-violet-50 border-b border-violet-100">
            <span class="font-serif-elegan text-2xl text-violet-800 font-bold">Menu</span>
            <button @click="open = false" class="w-8 h-8 flex items-center justify-center rounded-full border border-violet-200 hover:bg-violet-100 hover:text-red-600 transition text-slate-700">
        ...
        <div class="flex-1 overflow-y-auto py-4 px-5 space-y-2 bg-white">
            <template x-for="item in menu" :key="item.link">
                <a 
                    ...
                    class="flex items-center justify-between px-4 py-3 rounded-xl text-slate-600 font-medium hover:bg-violet-100 hover:text-violet-800 transition border border-transparent hover:border-violet-200"
                >
                    <span x-text="item.text"></span>
            ...
            {{-- CTA Button (Aksen Violet Tua) --}}
            <div class="mt-6 pt-6 border-t border-violet-100">
                <a href="#rsvp" @click="open = false" class="block w-full py-3 text-center bg-violet-800 text-white rounded-xl font-bold shadow-lg shadow-violet-300/40 hover:bg-violet-700 transition">
                    Kirim Ucapan
                </a>
            </div>
        </div>
        ...
        <div class="p-4 text-center bg-violet-50 text-xs text-slate-600 border-t border-violet-100">
