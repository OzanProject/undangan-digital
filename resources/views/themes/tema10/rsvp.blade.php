<div id="rsvp" class="container mx-auto px-4">
    ...
    <div class="grid md:grid-cols-12 gap-8 lg:gap-12 items-start">
        ...
        <div class="md:col-span-5 lg:col-span-5" data-aos="fade-right">
            <div class="bg-white rounded-3xl shadow-xl border border-fuchsia-200 p-6 md:p-8 sticky top-24">
                ...
                <form id="rsvp-form" method="POST" action="{{ route('wishes.store') }}#rsvp-form">
                    ...
                    <div class="mb-5">
                        ...
                        <input type="text" 
                                ...
                                {{-- Focus Style Magenta --}}
                                class="w-full px-5 py-3 rounded-xl bg-fuchsia-50 border-fuchsia-300 focus:bg-white focus:border-fuchsia-700 focus:ring-2 focus:ring-fuchsia-100 transition-all outline-none text-slate-800 font-medium placeholder:text-slate-400 placeholder:font-light"
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="message" 
                                 rows="4" 
                                 class="w-full px-5 py-3 rounded-xl bg-fuchsia-50 border-fuchsia-300 focus:bg-white focus:border-fuchsia-700 focus:ring-2 focus:ring-fuchsia-100 transition-all outline-none text-slate-800 font-medium placeholder:text-slate-400 placeholder:font-light resize-none"
                                 placeholder="Tuliskan doa terbaik untuk ananda..." 
                                 required>{{ old('message') }}</textarea>
                    </div>

                    {{-- Tombol Kirim (Aksen Magenta Tua) --}}
                    <button type="submit" class="w-full py-3.5 rounded-xl bg-fuchsia-700 text-white font-bold shadow-xl shadow-fuchsia-300/40 hover:bg-fuchsia-600 hover:-translate-y-0.5 transition-all duration-300 flex items-center justify-center gap-2">
                        <i class="bi bi-send-fill"></i> Kirim Ucapan
                    </button>

                    {{-- BAGIAN KHUSUS RSVP --}}
                    @if($guest)
                        <div class="mt-8 pt-6 border-t border-dashed border-fuchsia-200">
                            <p class="text-center text-sm font-bold text-slate-700 uppercase tracking-wider mb-4">
                                Konfirmasi Kehadiran
                            </p>
                            <div class="grid grid-cols-2 gap-3">
                                {{-- Tombol HADIR (Aksen Hijau/Mint dipertahankan karena menandakan status positif) --}}
                                <button type="submit" 
                                        formaction="{{ route('rsvp', $guest->id) }}#rsvp-form" 
                                        name="status" 
                                        value="hadir" 
                                        class="flex flex-col items-center justify-center py-3 px-2 rounded-xl border border-emerald-300 bg-emerald-100 text-emerald-700 hover:bg-emerald-700 hover:text-white hover:border-emerald-700 transition-all duration-300 group {{ $guest->status == 'hadir' ? 'bg-emerald-700 text-white border-emerald-700 shadow-md' : '' }}">
        ...
        <div class="md:col-span-7 lg:col-span-7" data-aos="fade-left">
            ...
            @endphp
            <div class="bg-white rounded-3xl shadow-xl border border-fuchsia-200 h-[600px] flex flex-col relative overflow-hidden">
                
                {{-- Header List (Aksen Magenta) --}}
                <div class="p-5 border-b border-fuchsia-200 bg-fuchsia-50 flex justify-between items-center">
                    <h3 class="font-bold text-slate-700">Ucapan Terbaru</h3>
                    {{-- Badge Jumlah Pesan (Aksen Mint) --}}
                    <span class="px-3 py-1 rounded-full bg-emerald-50 border border-emerald-300 text-xs font-bold text-emerald-700 shadow-sm">
                        {{ count($wishes) }} Pesan
                    </span>
                </div>
                ...
                <div class="flex-1 overflow-y-auto p-5 space-y-6 custom-scrollbar">
                    @forelse($wishes as $wish)
                        @php
                            ...
                            // Warna Avatar: Magenta/Mint/Netral
                            $colors = ['bg-fuchsia-200 text-fuchsia-800', 'bg-emerald-200 text-emerald-800', 'bg-slate-200 text-slate-700', 'bg-pink-200 text-pink-800', 'bg-teal-200 text-teal-800'];
                        ...
                        <div class="animate-fade-in-up">
                            <div class="flex gap-4">
                                ...
                                <div class="flex-1 space-y-2">
                                    {{-- Pesan Tamu (Netral) --}}
                                    <div class="bg-fuchsia-50 rounded-2xl rounded-tl-none p-4 border border-fuchsia-200 shadow-sm relative group">
                                    ...
                                    {{-- Balasan Admin (Aksen Mint) --}}
                                    @if($wish->reply)
                                        <div class="flex gap-3 justify-end animate-fade-in-up" style="animation-delay: 0.1s;">
                                            <div class="bg-emerald-200 rounded-2xl rounded-tr-none p-3 border border-emerald-300 shadow-sm max-w-[90%] text-right">
                    ...
                    @empty
                        <div class="h-full flex flex-col items-center justify-center text-fuchsia-600 opacity-60">
