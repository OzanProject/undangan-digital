<div id="rsvp" class="container mx-auto px-4">
    ...
    <div class="grid md:grid-cols-12 gap-8 lg:gap-12 items-start">
        ...
        <div class="md:col-span-5 lg:col-span-5" data-aos="fade-right">
            <div class="bg-white rounded-3xl shadow-xl border border-stone-200 p-6 md:p-8 sticky top-24">
                ...
                <form id="rsvp-form" method="POST" action="{{ route('wishes.store') }}#rsvp-form">
                    ...
                    <div class="mb-5">
                        ...
                        <input type="text" 
                                ...
                                {{-- Input Style Minimalis --}}
                                class="w-full px-5 py-3 rounded-xl bg-stone-100 border-stone-300 focus:bg-white focus:border-lime-700 focus:ring-2 focus:ring-lime-100 transition-all outline-none text-slate-800 font-medium placeholder:text-slate-400 placeholder:font-light"
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="message" 
                                 rows="4" 
                                 class="w-full px-5 py-3 rounded-xl bg-stone-100 border-stone-300 focus:bg-white focus:border-lime-700 focus:ring-2 focus:ring-lime-100 transition-all outline-none text-slate-800 font-medium placeholder:text-slate-400 placeholder:font-light resize-none"
                                 placeholder="Tuliskan doa terbaik untuk ananda..." 
                                 required>{{ old('message') }}</textarea>
                    </div>

                    {{-- Tombol Kirim (Aksen Zaitun) --}}
                    <button type="submit" class="w-full py-3.5 rounded-xl bg-lime-700 text-white font-bold shadow-xl shadow-lime-300/40 hover:bg-lime-600 hover:-translate-y-0.5 transition-all duration-300 flex items-center justify-center gap-2">
                        ...
                        <div class="mt-8 pt-6 border-t border-dashed border-stone-200">
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                ...
                                <button type="submit" 
                                        ...
                                        name="status" 
                                        value="hadir" 
                                        class="flex flex-col items-center justify-center py-3 px-2 rounded-xl border border-lime-300 bg-lime-100 text-lime-700 hover:bg-lime-700 hover:text-white hover:border-lime-700 transition-all duration-300 group {{ $guest->status == 'hadir' ? 'bg-lime-700 text-white border-lime-700 shadow-md' : '' }}">
        ...
        <div class="md:col-span-7 lg:col-span-7" data-aos="fade-left">
            <div class="bg-white rounded-3xl shadow-xl border border-stone-200 h-[600px] flex flex-col relative overflow-hidden">
                
                {{-- Header List (Aksen Abu-abu) --}}
                <div class="p-5 border-b border-stone-200 bg-stone-50 flex justify-between items-center">
                    <h3 class="font-bold text-slate-700">Ucapan Terbaru</h3>
                    {{-- Badge Jumlah Pesan (Aksen Abu-abu) --}}
                    <span class="px-3 py-1 rounded-full bg-white border border-stone-300 text-xs font-bold text-stone-700 shadow-sm">
                        {{ count($wishes) }} Pesan
                    </span>
                </div>
                ...
                <div class="flex-1 overflow-y-auto p-5 space-y-6 custom-scrollbar">
                    @forelse($wishes as $wish)
                        @php
                            ...
                            // Warna Avatar Minimalis: Abu-abu/Zaitun/Netral
                            $colors = ['bg-lime-100 text-lime-700', 'bg-stone-200 text-stone-700', 'bg-red-100 text-red-700', 'bg-blue-100 text-blue-700', 'bg-purple-100 text-purple-700'];
                        ...
                        <div class="animate-fade-in-up">
                            <div class="flex gap-4">
                                ...
                                <div class="flex-1 space-y-2">
                                    {{-- Pesan Tamu (Netral) --}}
                                    <div class="bg-stone-50 rounded-2xl rounded-tl-none p-4 border border-stone-200 shadow-sm relative group">
                                    ...
                                    {{-- Balasan Admin (Aksen Zaitun) --}}
                                    @if($wish->reply)
                                        <div class="flex gap-3 justify-end animate-fade-in-up" style="animation-delay: 0.1s;">
                                            <div class="bg-lime-50 rounded-2xl rounded-tr-none p-3 border border-lime-100 shadow-sm max-w-[90%] text-right">
                                                <div class="flex justify-end items-center gap-2 mb-1">
                                                    <span class="text-[10px] text-lime-700 font-bold uppercase tracking-wider">
                                                        Tuan Rumah
                                                    </span>
                                                    <i class="bi bi-check-circle-fill text-lime-500 text-xs"></i>
                                                </div>
                                                <p class="text-slate-700 text-sm leading-relaxed">
                                                    {{ $wish->reply }}
                                                </p>
                    ...
                    @empty
                        <div class="h-full flex flex-col items-center justify-center text-lime-600 opacity-60">
