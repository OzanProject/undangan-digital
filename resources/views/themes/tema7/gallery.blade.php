...
<div id="galeri" class="container mx-auto px-4">
            ...
            @foreach($galleries as $img)
                <div class="relative group" data-aos="zoom-in" data-aos-delay="{{ $loop->iteration * 100 }}">
                    ...
                    <a href="{{ asset('storage/'.$img->image) }}" 
                        ...
                        {{-- Shadow dan border Pink --}}
                        class="block w-full aspect-square overflow-hidden rounded-xl shadow-md cursor-pointer bg-stone-200 relative border border-pink-300">
    ...
    @else
        {{-- Empty State (Aksen Pink) --}}
        <div class="text-center py-12 bg-pink-50 rounded-3xl border border-dashed border-pink-300" data-aos="fade-up">
            <div class="inline-flex items-center justify-center w-16 h-16 rounded-full bg-pink-100 mb-4">
                <i class="bi bi-images text-3xl text-pink-500"></i>
            </div>
