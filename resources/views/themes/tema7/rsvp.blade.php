<div id="rsvp" class="container mx-auto px-4">
    ...
    <div class="text-center mb-12" data-aos="fade-up">
        <span class="text-pink-600 font-bold tracking-[0.2em] uppercase text-xs md:text-sm">
            Doa & Harapan
        </span>
        {{-- Font Sans --}}
    ...
    <div class="grid md:grid-cols-12 gap-8 lg:gap-12 items-start">
        ...
        <div class="md:col-span-5 lg:col-span-5" data-aos="fade-right">
            <div class="bg-white rounded-3xl shadow-xl border border-pink-200 p-6 md:p-8 sticky top-24">
                ...
                <form id="rsvp-form" method="POST" action="{{ route('wishes.store') }}#rsvp-form">
                    ...
                    <div class="mb-5">
                        ...
                        <input type="text" 
                                ...
                                {{-- Focus Style Pink Cerah --}}
                                class="w-full px-5 py-3 rounded-xl bg-pink-50 border-pink-300 focus:bg-white focus:border-pink-600 focus:ring-2 focus:ring-pink-100 transition-all outline-none text-slate-800 font-medium placeholder:text-slate-400 placeholder:font-light"
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="message" 
                                 rows="4" 
                                 class="w-full px-5 py-3 rounded-xl bg-pink-50 border-pink-300 focus:bg-white focus:border-pink-600 focus:ring-2 focus:ring-pink-100 transition-all outline-none text-slate-800 font-medium placeholder:text-slate-400 placeholder:font-light resize-none"
                                 placeholder="Tuliskan doa terbaik untuk ananda..." 
                                 required>{{ old('message') }}</textarea>
                    </div>

                    {{-- Tombol Kirim (Aksen Pink Tua) --}}
                    <button type="submit" class="w-full py-3.5 rounded-xl bg-pink-600 text-white font-bold shadow-xl shadow-pink-300/40 hover:bg-pink-500 hover:-translate-y-0.5 transition-all duration-300 flex items-center justify-center gap-2">
                        <i class="bi bi-send-fill"></i> Kirim Ucapan
                    </button>

                    {{-- BAGIAN KHUSUS RSVP --}}
                    @if($guest)
                        <div class="mt-8 pt-6 border-t border-dashed border-pink-200">
        ...
        <div class="md:col-span-7 lg:col-span-7" data-aos="fade-left">
            <div class="bg-white rounded-3xl shadow-xl border border-pink-200 h-[600px] flex flex-col relative overflow-hidden">
                
                {{-- Header List (Aksen Pink) --}}
                <div class="p-5 border-b border-pink-200 bg-pink-50 flex justify-between items-center">
                    <h3 class="font-bold text-slate-700">Ucapan Terbaru</h3>
                    {{-- Badge Jumlah Pesan (Aksen Pink) --}}
                    <span class="px-3 py-1 rounded-full bg-white border border-pink-300 text-xs font-bold text-pink-600 shadow-sm">
                        {{ count($wishes) }} Pesan
                    </span>
                </div>
                ...
                <div class="flex-1 overflow-y-auto p-5 space-y-6 custom-scrollbar">
                    @forelse($wishes as $wish)
                        @php
                            ...
                            // Warna Avatar: Pink/Netral
                            $colors = ['bg-pink-100 text-pink-700', 'bg-fuchsia-100 text-fuchsia-700', 'bg-slate-200 text-slate-700', 'bg-red-100 text-red-700', 'bg-rose-100 text-rose-700'];
                        ...
                        <div class="animate-fade-in-up">
                            <div class="flex gap-4">
                                ...
                                <div class="flex-1 space-y-2">
                                    ...
                                    {{-- Balasan Admin (Aksen Pink) --}}
                                    @if($wish->reply)
                                        <div class="flex gap-3 justify-end animate-fade-in-up" style="animation-delay: 0.1s;">
                                            <div class="bg-pink-200 rounded-2xl rounded-tr-none p-3 border border-pink-300 shadow-sm max-w-[90%] text-right">
                                                <div class="flex justify-end items-center gap-2 mb-1">
                                                    <span class="text-[10px] text-pink-700 font-bold uppercase tracking-wider">
                                                        Tuan Rumah
                                                    </span>
                                                    <i class="bi bi-check-circle-fill text-pink-500 text-xs"></i>
                                                </div>
                                                <p class="text-slate-700 text-sm leading-relaxed">
                                                    {{ $wish->reply }}
                                                </p>
                    ...
                    @empty
                        <div class="h-full flex flex-col items-center justify-center text-pink-600 opacity-60">
