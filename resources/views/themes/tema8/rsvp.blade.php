<div id="rsvp" class="container mx-auto px-4">
    ...
    <div class="grid md:grid-cols-12 gap-8 lg:gap-12 items-start">
        ...
        <div class="md:col-span-5 lg:col-span-5" data-aos="fade-right">
            <div class="bg-white rounded-3xl shadow-xl border border-teal-200 p-6 md:p-8 sticky top-24">
                ...
                <form id="rsvp-form" method="POST" action="{{ route('wishes.store') }}#rsvp-form">
                    ...
                    <div class="mb-5">
                        ...
                        <input type="text" 
                                ...
                                {{-- Focus Style Toska --}}
                                class="w-full px-5 py-3 rounded-xl bg-teal-50 border-teal-300 focus:bg-white focus:border-teal-600 focus:ring-2 focus:ring-teal-100 transition-all outline-none text-slate-800 font-medium placeholder:text-slate-400 placeholder:font-light"
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="message" 
                                 rows="4" 
                                 class="w-full px-5 py-3 rounded-xl bg-teal-50 border-teal-300 focus:bg-white focus:border-teal-600 focus:ring-2 focus:ring-teal-100 transition-all outline-none text-slate-800 font-medium placeholder:text-slate-400 placeholder:font-light resize-none"
                                 placeholder="Tuliskan doa terbaik untuk ananda..." 
                                 required>{{ old('message') }}</textarea>
                    </div>

                    {{-- Tombol Kirim (Aksen Toska Tua) --}}
                    <button type="submit" class="w-full py-3.5 rounded-xl bg-teal-600 text-white font-bold shadow-xl shadow-teal-300/40 hover:bg-teal-500 hover:-translate-y-0.5 transition-all duration-300 flex items-center justify-center gap-2">
                        <i class="bi bi-send-fill"></i> Kirim Ucapan
                    </button>

                    {{-- BAGIAN KHUSUS RSVP --}}
                    @if($guest)
                        <div class="mt-8 pt-6 border-t border-dashed border-teal-200">
        ...
        <div class="md:col-span-7 lg:col-span-7" data-aos="fade-left">
            ...
            @endphp
            <div class="bg-white rounded-3xl shadow-xl border border-teal-200 h-[600px] flex flex-col relative overflow-hidden">
                
                {{-- Header List (Aksen Toska) --}}
                <div class="p-5 border-b border-teal-200 bg-teal-50 flex justify-between items-center">
                    <h3 class="font-bold text-slate-700">Ucapan Terbaru</h3>
                    {{-- Badge Jumlah Pesan (Aksen Toska) --}}
                    <span class="px-3 py-1 rounded-full bg-white border border-teal-300 text-xs font-bold text-teal-700 shadow-sm">
                        {{ count($wishes) }} Pesan
                    </span>
                </div>
                ...
                <div class="flex-1 overflow-y-auto p-5 space-y-6 custom-scrollbar">
                    @forelse($wishes as $wish)
                        @php
                            ...
                            // Warna Avatar: Toska/Biru/Netral
                            $colors = ['bg-teal-100 text-teal-700', 'bg-sky-100 text-sky-700', 'bg-slate-200 text-slate-700', 'bg-cyan-100 text-cyan-700', 'bg-blue-100 text-blue-700'];
                        ...
                        <div class="animate-fade-in-up">
                            <div class="flex gap-4">
                                ...
                                <div class="flex-1 space-y-2">
                                    {{-- Pesan Tamu (Netral) --}}
                                    <div class="bg-teal-50 rounded-2xl rounded-tl-none p-4 border border-teal-200 shadow-sm relative group">
                                    ...
                                    {{-- Balasan Admin (Aksen Toska) --}}
                                    @if($wish->reply)
                                        <div class="flex gap-3 justify-end animate-fade-in-up" style="animation-delay: 0.1s;">
                                            <div class="bg-sky-100 rounded-2xl rounded-tr-none p-3 border border-sky-200 shadow-sm max-w-[90%] text-right">
                                                <div class="flex justify-end items-center gap-2 mb-1">
                                                    <span class="text-[10px] text-sky-700 font-bold uppercase tracking-wider">
                                                        Tuan Rumah
                                                    </span>
                                                    <i class="bi bi-check-circle-fill text-sky-500 text-xs"></i>
                                                </div>
                                                <p class="text-slate-700 text-sm leading-relaxed">
                                                    {{ $wish->reply }}
                                                </p>
                    ...
                    @empty
                        <div class="h-full flex flex-col items-center justify-center text-teal-600 opacity-60">
