...
>
    {{-- Overlay Hitam/Sangat Gelap --}}
    <div class="absolute inset-0 bg-black/70 bg-gradient-to-b from-black/95 via-black/50 to-black/95 z-0"></div>
    ...
    >
        ...
        <div data-aos="fade-down" data-aos-duration="1000" class="w-full text-center mt-1">
            <p class="text-cyan-100 text-[10px] md:text-xs font-medium tracking-[0.3em] uppercase mb-2">
                Kepada Yth. Bapak/Ibu/Saudara/i
            </p>
            
            {{-- Badge: background transparan gelap --}}
            <div class="inline-block bg-slate-800/50 backdrop-blur-sm border border-cyan-200/50 rounded-full px-6 py-2 shadow-2xl">
        ...
        <div class="w-full px-2 text-center">
            ...
            {{-- Nama: Font Sans dan Aksen Cyan Neon --}}
            <h1 class="font-sans text-4xl md:text-5xl lg:text-6xl text-cyan-300 mt-2 mb-3 md:mb-5 drop-shadow-2xl" 
                data-aos="zoom-in" 
                data-aos-delay="400"
                {{-- Efek shadow neon (Cyan) --}}
                style="text-shadow: 0 0 25px rgba(6, 182, 212, 0.8); line-height: 1.1; font-weight: 700;"
            >
                {{ $event->child_name }}
            </h1>
        </div>
    ...
    {{-- D. TOMBOL BUKA UNDANGAN (Aksen Cyan Tua) --}}
    <div class="absolute inset-x-0 bottom-6 flex justify-center z-20 px-4">
        <a 
            ...
            class="btn-buka btn-buka-enter group relative inline-flex items-center justify-center 
                    ...
                    transition-all duration-300 bg-cyan-600 rounded-full 
                    hover:bg-cyan-500 hover:shadow-[0_0_25px_rgba(6,182,212,0.6)] 
                    hover:-translate-y-1 active:scale-95 shadow-xl shadow-cyan-900/50"
        >
