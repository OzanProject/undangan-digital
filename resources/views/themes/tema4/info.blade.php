...
<div id="acara" class="container mx-auto px-4">
    ...
    {{-- Main Card --}}
    {{-- Card Background: slate-800, Border: slate-700 --}}
    <div class="max-w-5xl mx-auto bg-slate-800 rounded-3xl shadow-2xl overflow-hidden border border-slate-700 flex flex-col md:flex-row" 
        data-aos="fade-up" data-aos-delay="100">
        
        {{-- Kiri: Detail Informasi --}}
        <div class="w-full md:w-5/12 p-8 md:p-10 flex flex-col justify-center bg-slate-800 relative z-10">
            
            <h3 class="font-sans text-3xl text-cyan-400 mb-8 text-center md:text-left font-bold">
                Syukuran Khitan
            </h3>
            ...
            <div class="space-y-8">
                ...
                <div class="flex items-start gap-4">
                    {{-- Ikon: Aksen Cyan --}}
                    <div class="flex-shrink-0 w-12 h-12 rounded-xl bg-slate-700 text-cyan-400 flex items-center justify-center shadow-inner shadow-cyan-900/50">
                ...
                <div class="flex items-start gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-lg font-bold text-white leading-tight mb-1">
                            {{ $event->location_name }}
                        </p>
            ...
            <div class="mt-10 space-y-3">
                ...
                {{-- 1. Tombol Google Maps (Aksen Cyan Tua) --}}
                <a href="https://maps.google.com/?q={{ urlencode($event->location_name . ' ' . $event->location_address) }}" 
                    target="_blank"
                    class="block w-full text-center py-3 px-6 rounded-xl bg-cyan-600 text-white font-semibold shadow-xl shadow-cyan-900/40 hover:bg-cyan-500 hover:shadow-2xl transition-all transform hover:-translate-y-0.5 group">
                ...
                {{-- 2. Tombol Add to Calendar (Aksen Cyan) --}}
                <div class="grid grid-cols-2 gap-3">
                    {{-- Google Calendar --}}
                    <a href="{!! $googleUrl !!}" target="_blank"
                        class="flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl bg-slate-700 border border-slate-600 text-slate-300 text-sm font-bold hover:bg-slate-600 hover:text-cyan-400 hover:border-cyan-400 transition-all">
        ...
        <div class="w-full md:w-7/12 h-80 md:h-auto relative bg-slate-900">
            @if($event->maps_iframe)
                <iframe 
                    src="{{ $event->maps_iframe }}" 
                    class="w-full h-full border-0 filter grayscale invert hover:grayscale-0 hover:invert-0 transition-all duration-700" 
                    allowfullscreen="" 
                    loading="lazy" 
                    referrerpolicy="no-referrer-when-downgrade">
                </iframe>
            @else
                <div class="w-full h-full flex flex-col items-center justify-center text-slate-500 space-y-2 p-6 text-center">
                    <i class="bi bi-map-fill text-4xl opacity-50"></i>
                    <span>Peta belum disetting oleh admin.</span>
                </div>
            @endif
            
            {{-- Efek fade di perbatasan dengan card detail --}}
            <div class="absolute top-0 bottom-0 left-0 w-16 bg-gradient-to-r from-slate-800 to-transparent hidden md:block pointer-events-none"></div>
        </div>
